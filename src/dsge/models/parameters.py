...
class Prior:
    ...
    def __post_init__(self):
        ...
        if self.distribution not in valid_dists:
            msg = f"Distribution must be one of {valid_dists}"
    ...
    def logpdf(self, x: float) -> float:
        ...
        if self.distribution == "uniform":
            return stats.uniform.logpdf(
                x, loc=self.params["lower"], scale=self.params["upper"] - self.params["lower"]
            )
        if self.distribution == "invgamma":
...
class ParameterSet:
    ...
    def set_values(self, values: np.ndarray) -> None:
        ...
        for p, v in zip(self._params.values(), values, strict=False):
            if not p.fixed:
                p.value = v
    ...
    def log_prior(self, values: np.ndarray | None = None) -> float:
