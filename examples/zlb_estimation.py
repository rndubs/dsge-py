...
class SimpleZLBModel(DSGEModel):
    ...
    def _setup_parameters(self) -> None:
        ...
        self.parameters.add(
            Parameter(
                name="r_star", value=0.02, description="Natural rate (steady state)", fixed=True
            )
        )
...
def main() -> None:
    ...
    # Solve both regimes
    sys_normal = model_normal.system_matrices()
    solution_normal, _info_normal = solve_linear_model(
        sys_normal["Gamma0"], sys_normal["Gamma1"], sys_normal["Psi"], sys_normal["Pi"], n_states=1
    )
    solution_normal.Q = model_normal.shock_covariance()

    sys_zlb = model_zlb.system_matrices()
    solution_zlb, _info_zlb = solve_linear_model(
        sys_zlb["Gamma0"], sys_zlb["Gamma1"], sys_zlb["Psi"], sys_zlb["Pi"], n_states=1
    )
    ...
    axes[1].plot(
        filter_results.regime_sequence,
        "r--",
        linewidth=2,
        label="KF Inferred",
    ...
    axes[2].fill_between(
        range(T),
        0,
        pf_results.regime_probabilities[:, 0],
        alpha=0.3,
        label="P(Normal)",
        color="blue",
    )
    axes[2].fill_between(
        range(T), 0, pf_results.regime_probabilities[:, 1], alpha=0.3, label="P(ZLB)", color="red"
    )
