...
from dsge.forecasting import forecast_from_posterior, forecast_observables
...
def forecast_nyfed_model(
    ...
    posterior_path: str | None = "results/nyfed_estimation/posterior_samples.csv",
    horizon: int = 20,
    n_paths: int = 1000,
    ...
    kf_result = kalman_filter(y=y, T=T_mat, R=R_mat, Z=Z, D=D, Q=Q, H=H, a0=x0, P0=P0)
    ...
    if use_posterior and posterior_path:
        ...
        forecast_result = forecast_observables(
            T=T_mat, R=R_mat, C=C_vec, Z=Z, D=D, x_T=x_T, horizon=horizon, n_paths=n_paths, seed=42
        )
    ...
    forecast_dates = pd.date_range(last_date + pd.DateOffset(months=3), periods=horizon, freq="Q")
    ...
    mean_df = pd.DataFrame(
        forecast_result.mean, index=forecast_dates, columns=model.spec.observable_names
    )
    ...
    if forecast_result.bands:
        for level, (lower, upper) in forecast_result.bands.items():
            lower_df = pd.DataFrame(
                lower, index=forecast_dates, columns=model.spec.observable_names
            )
            upper_df = pd.DataFrame(
                upper, index=forecast_dates, columns=model.spec.observable_names
            )
    ...
    _fig, axes = plt.subplots(len(key_vars), 1, figsize=(12, 8))
    ...
    for i, var in enumerate(key_vars):
        ...
        # Forecast mean
        ax.plot(
            forecast_dates, forecast_result.mean[:, var_idx], "b-", linewidth=2, label="Forecast"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--posterior", type=str, default=None, help="Path to posterior samples CSV")
    parser.add_argument("--horizon", type=int, default=20, help="Forecast horizon in quarters")
    parser.add_argument("--paths", type=int, default=1000, help="Number of simulation paths")
    parser.add_argument(
        "--output", type=str, default="results/nyfed_forecasts", help="Output directory"
    )
    parser.add_argument(
        "--use-posterior", action="store_true", help="Incorporate posterior uncertainty"
    )
