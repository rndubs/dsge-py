...
def plot_irfs(irfs, state_idx, vars_to_plot=None, save_path=None):
    ...
    plt.suptitle(
        "Impulse Response Functions - NYFed Model 1002", fontsize=16, fontweight="bold", y=1.00
    )
    plt.tight_layout()
...
def plot_simulations(states, state_idx, vars_to_plot=None, save_path=None):
    ...
    plt.suptitle("Simulated Paths - NYFed Model 1002", fontsize=16, fontweight="bold", y=1.00)
    plt.tight_layout()
...
def create_validation_report(solution, model, info, save_dir="validation"):
    ...
    fig_irf = plot_irfs(
        irfs, state_idx, vars_to_plot=vars_to_plot, save_path=f"{save_dir}/irfs.png"
    )
    plt.close(fig_irf)
    ...
    fig_sim = plot_simulations(
        states, state_idx, vars_to_plot=vars_to_plot, save_path=f"{save_dir}/simulation.png"
    )
    plt.close(fig_sim)
