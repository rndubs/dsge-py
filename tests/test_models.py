...
from dsge import ModelSpecification, Parameter, ParameterSet, Prior


def test_prior_normal() -> None:
    """Test normal prior."""
...
def test_parameter() -> None:
    ...
    param = Parameter(
        name="alpha", value=0.33, bounds=(0, 1), prior=Prior("beta", {"alpha": 3.3, "beta": 6.7})
    )
...
def test_parameter_set_log_prior() -> None:
    """Test log prior evaluation."""
    params = ParameterSet()
    params.add(Parameter(name="alpha", value=0.5, prior=Prior("beta", {"alpha": 2.0, "beta": 2.0})))

    log_prior = params.log_prior()
    assert np.isfinite(log_prior)


def test_model_specification() -> None:
