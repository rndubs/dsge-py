...
class NYFedModel1002(DSGEModel):
    ...
    def __init__(self):
        ...
        state_names = [
            ...
            'c', 'i', 'y', 'L', 'k_bar', 'k', 'u', 'q_k', 'w', 'R', 'pi', 'mc',
            'r_k', 'R_k_tilde', 'n', 'w_h', 'y_f', 'pi_star_t',
            # Lags
            'c_lag', 'i_lag', 'w_lag', 'R_lag', 'pi_lag', 'k_bar_lag', 'q_k_lag',
            'n_lag', 'y_lag', 'y_f_lag',
            # Shocks
            'z_tilde', 'z_p', 'b', 'mu', 'g', 'lambda_f', 'lambda_w', 'sigma_omega', 'r_m',
            # Shock MA terms
            'lambda_f_ma', 'lambda_w_ma',
            # Measurement errors
            'e_gdp', 'e_gdi', 'e_pce', 'e_gdpdef', 'e_10y', 'e_tfp',
        ...
        shock_names = [
            'eps_z', 'eps_zp', 'eps_b', 'eps_mu', 'eps_g',
            'eps_lambda_f', 'eps_lambda_w', 'eps_sigma_omega', 'eps_rm'
        ]
    ...
    def _setup_parameters(self):
        ...
        for var in me_vars:
            self.parameters.add(Parameter(
                ...
                prior=make_prior('normal' if var in ['gdp', 'gdi'] else 'beta', 0.00 if var in ['gdp', 'gdi'] else 0.50, 0.20),
                fixed=False,
    ...
    def _compute_steady_state_ratios(self, p: Dict[str, float]) -> Dict[str, float]:
        ...
        g_star = p['g_star']
        Phi_p = p['Phi_p']
        SP_star = p['SP_star'] / 400  # Convert annual spread to quarterly
        gamma_ent = p['gamma_star']  # Entrepreneur survival rate
        ...
        R_k_annual = R_k_star  # Already gross return
    ...
    def system_matrices(self, params: Optional[np.ndarray] = None) -> Dict[str, np.ndarray]:
        ...
        eta_names = ['eta_c', 'eta_i', 'eta_pi', 'eta_w', 'eta_R_k_tilde', 'eta_L', 'eta_y',
                     'eta_R', 'eta_q_k', 'eta_n', 'eta_z', 'eta_pi_star_t', 'eta_y_f']
        ...
        #       + lambda_f[t]
        # ===================================================================
        kappa = (1 - zeta_p * beta) * (1 - zeta_p) / ((1 + iota_p * beta_bar) * zeta_p * ((Phi_p - 1) * epsilon_p + 1))
        ...
        #      + lambda_w[t]
        # ===================================================================
        kappa_w = (1 - zeta_w * beta) * (1 - zeta_w) / ((1 + beta_bar) * zeta_w * ((lambda_w - 1) * epsilon_w + 1))
        ...
        # LAG DEFINITIONS
        # ===================================================================
        for var, var_lag in [('c', 'c_lag'), ('i', 'i_lag'), ('w', 'w_lag'), ('R', 'R_lag'),
                             ('pi', 'pi_lag'), ('k_bar', 'k_bar_lag'), ('q_k', 'q_k_lag'),
                             ('n', 'n_lag'), ('y', 'y_lag'), ('y_f', 'y_f_lag')]:
        ...
        for me_var, (rho_me, sigma_me) in me_params.items():
...
if __name__ == "__main__":
    ...
    print(f"Model: NYFed DSGE Model 1002")
    ...
    try:
        ...
        print(f"\nSystem matrices created successfully:")
        ...
        print(f"\nError creating system matrices: {e}")
        import traceback
        traceback.print_exc()
    ...
    try:
        ...
        print(f"\nMeasurement equation created successfully:")
        print(f"  Z shape: {Z.shape}")
        print(f"  D shape: {D.shape}")
    except Exception as e:
        print(f"\nError creating measurement equation: {e}")
        import traceback
        traceback.print_exc()
